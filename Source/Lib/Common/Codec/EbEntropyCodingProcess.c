    ...
    uint32_t               slide_win_length)
{
    ...
    float *propagate_weight_array[slide_win_length];
    ...
    for(int frame = 0; frame < slide_win_length; frame++) {
        //EB_FREE_ARRAY(propagate_weight_array[frame]);
        EB_MALLOC_ARRAY(propagate_weight_array[frame], block_total_count);
        for(int i=0; i<block_total_count; i++)
            propagate_weight_array[frame][i] = 1.0;
    }

    for(int frame=0; frame < slide_win_length; frame++) {
        stat_info_struct_t *stat_info_struct = sequence_control_set_ptr->stat_info_struct[(decode_order - frame) % STAT_LA_LENGTH];
        float *propagate_weight = propagate_weight_array[slide_win_length - frame - 1];
        for(int block_index=0; block_index < block_total_count; block_index++) {
            for(int sb_index=0; sb_index < stat_info_struct[block_index].ref_sb_cnt; sb_index++) {
                ...
                {
                    float *ref_propagate_weight = propagate_weight_array[ref_decode_order - stat_queue_head_index - 1];
                    //ref_propagate_weight[stat_info_struct[block_index].ref_sb_index[sb_index]] += (propagate_weight[block_index] * propagate_frac);
                    assert(stat_info_struct[block_index].ref_sb_index[sb_index]<block_total_count);
    ...
    if(stat_queue_head_index==1)
    {
        for(int frame=0; frame < slide_win_length; frame++) {
            float *propagate_weight = propagate_weight_array[slide_win_length - frame - 1];
            printf("kelvin ---> propagate weight frame %d in B1\n", frame);
    ...
    for(int frame=0; frame < slide_win_length; frame++) {
        ...
        stat_info_struct_t *stat_info_struct = sequence_control_set_ptr->stat_info_struct[curr_decode_order];
        assert((curr_decode_order-stat_queue_head_index-1)>=0 && (curr_decode_order-stat_queue_head_index-1)<slide_win_length);
        float *propagate_weight = propagate_weight_array[curr_decode_order - stat_queue_head_index - 1];
        //printf("kelvin ---> stat_queue_head_index=%d, frame=%d, curr_decode_order - stat_queue_head_index - 1=%d\n", stat_queue_head_index, frame, curr_decode_order - stat_queue_head_index - 1);
        uint16_t temporal_weight = sequence_control_set_ptr->temporal_weight[curr_decode_order];
        for(int block_index=0; block_index < block_total_count; block_index++) {
            for(int sb_index=0; sb_index < stat_info_struct[block_index].ref_sb_cnt; sb_index++) {
